...
public class DeleteMatchEventTypeCommandHandlerTests
{
    private readonly Mock<IMatchEventTypeRepository> _typeRepository = new();
    private readonly DeleteMatchEventTypeCommandHandler _handler;

    public DeleteMatchEventTypeCommandHandlerTests()
    {
        _handler = new DeleteMatchEventTypeCommandHandler(_typeRepository.Object);
    }
    ...
    public async Task Handle_NotFound_ShouldReturnNotFound()
    {
        ...
        var result = await _handler.HandleAsync(new DeleteMatchEventTypeCommand(Guid.NewGuid()));
    ...
    [Fact]
    public async Task Handle_Existing_ShouldDeactivateType()
    {
        ...
        var result = await _handler.HandleAsync(new DeleteMatchEventTypeCommand(type.Id));
