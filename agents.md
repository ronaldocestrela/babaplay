...
*   Policies usadas em `<AuthorizeView Policy="...">` devem ser registradas em `AddAuthorizationCore` no Web (`ClientAuthorizationPolicies` + handler client-side). A UI carrega permissões RBAC do tenant via `GET /api/v1/auth/me/permissions` em `TenantState.Permissions` (login, seleção de tenant e restore de sessão). `CommunicationWriteView` e a policy client-side liberam escrita quando o usuário tem `communication.write` ou é owner; a API continua validando RBAC de verdade.
